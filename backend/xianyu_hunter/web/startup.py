...
def start_kb_refresh_scheduler(container: Any) -> None:
    ...
    独立于批量采集调度器：本调度器仅扫描 docs/ 和 backend/ 目录构建知识库，
...
def _run_tunnel_autostart(container: Any) -> None:
    ...
    try:
        ...
        # Tailscale 未安装/未登录属"可预期的未配置状态"，降级为 WARNING 并抑制完整堆栈
        # （logger.exception 会附带 38 行 traceback，误导为系统故障；仅记录 message 即可）
        logger.warning("内网穿透自动启动失败: {}（如需使用请安装并登录 Tailscale 后重试）", e)

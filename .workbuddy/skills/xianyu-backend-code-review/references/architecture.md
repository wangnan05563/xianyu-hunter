...
## 十三、v4.65.0 新增检查点速查（B-REVIEW-316~319，维度 38 架构层依赖强化）

> **沉淀来源**：`_skill_optim/architecture-rule.md`（4 条规则全部沉淀为 B-REVIEW 检查点）
> **配置节点**：`config.yaml#architecture_layering`（4 个子节点）
> **完整描述**：详见 [SKILL.md §38 架构层依赖强化](file:///d:/code/otherProjects/17_xianyu/.trae/skills/xianyu-backend-code-review/SKILL.md) 维度正文

### 13.1 检查点清单

| ID | 名称 | 严重等级 | 与现有章节的关系 |
|---|---|---|---|
| B-REVIEW-316 | LAYER-DEPENDENCY-DIRECTION 层依赖方向铁律 | error | 强化 §2.2 规则表，新增 grep 检测信号 |
| B-REVIEW-317 | LIBS-BUSINESS-AGNOSTIC 公共工具库业务无关 | error | 全新检查点，§3 模块职责未覆盖 |
| B-REVIEW-318 | BUSINESS-LOGIC-OUT-OF-CONTROLLER 路由业务剥离 | error | 强化 §4.2 关键检查点，新增行数阈值 |
| B-REVIEW-319 | REPOSITORY-ABSTRACTION-REUSE 仓储抽象复用 | warning | 强化 §3.2 仓储规范，新增绕过检测 |

### 13.2 判断信号速查

```bash
# B-REVIEW-316 层依赖方向违规检测
...
grep -rn "from xianyu_hunter\.web import" src/xianyu_hunter/(domain|infra|modules)/

# B-REVIEW-317 公共工具库业务无关违规检测
grep -rn "from xianyu_hunter\.modules\." src/xianyu_hunter/infra/utils/
grep -rn "from xianyu_hunter\.web\.services\." src/xianyu_hunter/infra/utils/

# B-REVIEW-318 路由层业务逻辑剥离违规检测
grep -rn "session\.execute" src/xianyu_hunter/web/routes/
grep -rn "session\.commit" src/xianyu_hunter/web/routes/

# B-REVIEW-319 仓储抽象复用违规检测
grep -rn "session\.execute(select(" src/xianyu_hunter/modules/
grep -rn "session\.execute(update(" src/xianyu_hunter/modules/
```
...
| 检查点 | 适用 | 不适用 |
|---|---|---|
| B-REVIEW-316 | 所有 `src/xianyu_hunter/` 下的 Python 文件 | `tests/` 目录；第三方库 |
| B-REVIEW-317 | `infra/utils/`、`infra/helper/`、`infra/common/` 等公共工具目录 | `infra/repo_*.py`（仓储可依赖 domain）；`infra/container.py`（依赖注入容器可依赖各层） |
| B-REVIEW-318 | 所有 `web/routes/api_*.py` 路由函数 | 路由层中间件；路由层参数校验；简单 CRUD 端点 |
| B-REVIEW-319 | 所有已有仓储抽象的表 | 无仓储抽象的表；仓储内部实现；一次性脚本 |

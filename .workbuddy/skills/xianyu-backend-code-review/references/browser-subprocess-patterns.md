...
## 八、async_timeout_protection 审查要点（v4.48.0）

> 本章对应 `xianyu-hunter-dev` v4.48.0 meta-rules #80/#81/#82，配套 `config.yaml` 的 `async_timeout_protection` / `subprocess_heartbeat` / `consistency_check` 三个节点。背景：浏览器登录成功后 Cookie 导出阶段 `context.cookies()` 与 `bc.storage_state()` 缺少 `asyncio.wait_for` 超时保护，Playwright IPC 阻塞导致子进程心跳停滞，后端 90s 后误判卡死并 kill 子进程，丢失已登录 Cookie。

### 8.1 B-REVIEW-226 检查清单：async 阻塞调用超时保护

**审计目标**：所有可能永久挂起的 async API 调用必须有 `asyncio.wait_for` 超时保护，超时值按调用成本分类从 config 读取。

**Grep 命令**（命中即疑似违规，需人工核对是否有 `asyncio.wait_for` 包裹）：

```bash
# 扫描所有 Python 文件中无超时保护的 Playwright 阻塞 API 调用
grep -rnP "await\s+\w+\.(cookies|storage_state|snapshot|title|url|evaluate)\(" \
  scripts/ src/xianyu_hunter/ --include="*.py"
```

**反例（违规）**：

```python
# scripts/browser_login.py - Cookie 导出阶段
async def export_cookies(context, bc):
    # 违规：context.cookies() 与 bc.storage_state() 在浏览器 IPC 阻塞时会永久挂起
...
```python
# scripts/browser_login.py - Cookie 导出阶段（修复后）
import asyncio
from xianyu_hunter.config import load_config

cfg = load_config()
_async_cfg = cfg.async_timeout_protection

async def export_cookies(context, bc):
    # heavy_serialize 类（10s）：Playwright 跨 IPC 序列化所有 cookies + storage
    timeout = _async_cfg.timeout_by_category.heavy_serialize
    try:
        ...
        # fallback_strategy: best_effort - 返回空 cookies 并告警，不让子进程卡死
        logger.warning("cookie export timed out after %.1fs, fallback to empty", timeout)
        return [], {}
```

**修复模式**：
1. 调用成本分类：`lightweight_read`（page.title/url，5s）< `heavy_serialize`（cookies/storage_state/snapshot，10s）< `evaluate`（page.evaluate，15s，可能跑任意 JS）
2. 超时值从 `async_timeout_protection.timeout_by_category` 读取，禁止硬编码
3. 超时后按 `fallback_strategy` 处理：`best_effort`（返回空值 + 告警，不让子进程卡死）/ `abort`（抛异常让上层处理）
4. 不适用：`page.goto(timeout=...)` / `wait_for_load_state(timeout=...)` 已自带 timeout 参数，无需包裹

### 8.2 B-REVIEW-227 检查清单：子进程心跳与阶段超时协同

**审计目标**：`subprocess.Popen` + status file 心跳架构中，每阶段独立 status + 独立超时阈值，阶段内阻塞调用累计超时 < 心跳阈值 × (1 - safety_margin)。

**阶段超时计算公式**：

```
阶段允许累计超时 = 该阶段心跳阈值 × (1 - safety_margin)

例如 already_logged 阶段：
  心跳阈值 = subprocess_heartbeat.timeout_by_status.already_logged = 90s
  safety_margin = 0.3
  阶段允许累计超时 = 90 × (1 - 0.3) = 63s

阶段内阻塞调用累计超时（按 asyncio.wait_for 的 timeout 参数求和）：
  - context.cookies()        timeout = 10s (heavy_serialize)
  - bc.storage_state()       timeout = 10s (heavy_serialize)
  - page.evaluate("...")     timeout = 15s (evaluate)
  累计 = 35s < 63s  ✅ 合规

若再叠加一次 page.snapshot() timeout=10s：
  累计 = 45s < 63s  ✅ 合规但接近阈值，触发 max_accumulated_timeout_warn (30s) 告警
```

**心跳与超时协同验证清单**：
1. 每阶段 status 字符串必须在 `subprocess_heartbeat.timeout_by_status` 中有对应阈值
2. 子进程进入阶段时必须写入 status file（后端轮询依据）
3. 阶段切换时 status file 必须更新（避免后端按上一阶段阈值判定超时）
4. 阶段内所有 `asyncio.wait_for` 的 timeout 之和必须 < 阈值 × (1 - safety_margin)
5. 累计超时 > `max_accumulated_timeout_warn` 时应记录 warning 日志（便于回归审计）

**反例（违规）**：

```python
# 子进程只定义单一 running 状态，所有阶段共用 90s 阈值
def write_status(stage):
    # 违规：阶段未区分，全部写 "running"
    status_file.write_text("running")

# already_logged 阶段累计超时 80s > 90 × 0.7 = 63s
async def export_all(context, bc, page):
    cookies = await asyncio.wait_for(context.cookies(), timeout=30)  # 30s
    storage = await asyncio.wait_for(bc.storage_state(), timeout=30)  # 30s
    snapshot = await asyncio.wait_for(page.snapshot(), timeout=20)   # 20s
    # 累计 80s > 63s，且后端按 running 阈值 90s 判定，可能在 80s 处误判卡死
```
...
from xianyu_hunter.config import load_config
cfg = load_config()
_hb_cfg = cfg.subprocess_heartbeat

# 每阶段独立 status + 独立阈值
def enter_stage(stage: str):
    if stage not in _hb_cfg.timeout_by_status:
        raise ValueError(f"unknown stage: {stage}")
    status_file.write_text(stage)  # 后端按 stage 查 timeout_by_status[stage] 判定

async def export_all(context, bc, page):
    enter_stage("already_logged")  # 阈值 90s，允许累计 63s
    cookies = await asyncio.wait_for(context.cookies(), timeout=10)   # 10s
    storage = await asyncio.wait_for(bc.storage_state(), timeout=10) # 10s
    snapshot = await asyncio.wait_for(page.snapshot(), timeout=10)   # 10s
    # 累计 30s < 63s  ✅ 合规
```

### 8.3 B-REVIEW-228 检查清单：跨代码块一致性检查

**审计目标**：同一 API 在 ≥ `consistency_check.min_occurrences`（默认 2）处调用时，保护措施必须一致，以更严格者为准。
...
cfg = load_config()
_cc = cfg.consistency_check

# 关注的 API 模式（与 B-REVIEW-226 的 audit_grep_pattern 对齐）
API_PATTERN = re.compile(r"await\s+\w+\.(cookies|storage_state|snapshot|title|url|evaluate)\(")
# 是否有 asyncio.wait_for 包裹（向上回看 5 行内是否出现 asyncio.wait_for）
WAIT_FOR_PATTERN = re.compile(r"asyncio\.wait_for\s*\(")
...
def scan_file(path: Path):
    ...
    for i, line in enumerate(lines):
        ...
        if not m:
            continue
        api_name = m.group(1)
        # 向上回看 5 行，判断是否有 asyncio.wait_for 包裹
        lookback = "\n".join(lines[max(0, i-5):i+1])
        has_protection = bool(WAIT_FOR_PATTERN.search(lookback))
        findings.append({
            "file": str(path),
            "line": i + 1,
            "api": api_name,
            "has_wait_for": has_protection,
        })
    return findings

def main():
    all_findings = {}
    for rel in _cc.scan_paths:
        p = Path(rel)
        if not p.exists():
            continue
        for f in scan_file(p):
            all_findings.setdefault(f["api"], []).append(f)

    for api, calls in all_findings.items():
        if len(calls) < _cc.min_occurrences:
            continue
        protections = {c["has_wait_for"] for c in calls}
        if len(protections) > 1:
            print(f"[INCONSISTENT] api={api}, calls={len(calls)}:")
            for c in calls:
                flag = "✅" if c["has_wait_for"] else "❌"
                print(f"  {flag} {c['file']}:{c['line']}")
            # 以更严格者为准：要求所有调用点都有 wait_for
            print(f"  → 修复策略：补齐所有 ❌ 行的 asyncio.wait_for 保护")
...
**不一致修复策略**：
1. **以更严格者为准**：若一处有 `asyncio.wait_for` 而另一处无，则无保护处必须补齐至与有保护处一致（不能反向「为了统一而移除保护」）
2. **超时值统一**：同一 API 多处调用的 timeout 值必须从同一 config 字段读取，禁止一处用 `lightweight_read` 另一处用 `heavy_serialize`
3. **fallback 行为统一**：同一 API 多处调用的 `except asyncio.TimeoutError` 处理必须一致（要么都 best_effort，要么都 abort）
4. **重构后必扫**：任何对 `storage_state()` / `context.cookies()` 调用点的修改都必须触发一致性扫描，避免漏改

**反例（违规）**：

```python
# scripts/browser_login.py - 主登录流程（有保护）
async def login_and_export(context, bc):
    cookies = await asyncio.wait_for(context.cookies(), timeout=10)  # ✅
    return cookies

# scripts/auth_helper.py - 快速导出路径（无保护，违规）
async def quick_export(context, bc):
...
| v1.1 | 2026-07-18 | 新增第八章 async_timeout_protection 审查要点（B-REVIEW-226/227/228） |

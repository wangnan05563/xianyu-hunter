# 维度 48：重定向/中间件不得破坏反代（Funnel 回环安全）🆕v4.71.0 · F-REVIEW-247

> **编码规范引用**：xianyu-hunter-dev `references/coding-standards-2026-08-13.md` §11.2 / meta-rule #120
> **配置节点**：config.yaml#proxy_redirect_safety + config.yaml#spa_basename_consistency
> **测试关联**：xianyu-auto-testing 模式 AL（子路径部署一致性）；xianyu-backend-code-review 维度 48（后端侧交叉）
...
- 修复「根路径白板」靠「统一入口指向子路径」（`config.yaml#spa_basename_consistency.forbidden_entry_prefixes` 之外的正确入口），而非加重定向。

### 推荐（P1 严重）
- 重定向目标不得使 SPA `basename` 失配（与维度 45 三处对齐联动检查）。
...
```bash
# 检测是否存在可能破坏反代的重定向
grep -rn "redirect\|307\|RedirectResponse\|/xianyu/\"" src/xianyu_hunter/web/app.py
...
- 症状「根路径白板但 `/xianyu/` 正常」→ 入口契约失配，修复入口而非加重定向

...
| v4.1.0 | F-REVIEW-SSE-ERROR-HANDLING | 15 | 36 个8 | SSE错误码分、|
...
| v4.52.0 | F-REVIEW-197 ACTION-BUTTON-TIER-RETAIN / F-REVIEW-198 MULTI-VIEW-MODE-CONSISTENCY | 46 | 107->109 | UI 操作项分级保留 / 多视图模式一致性 (meta-rule #83 落地, dim 46) |
...
- 🆕v4.16【强制、*F-REVIEW-STATE-FUNCTIONAL-ALIGN：状态显示与功能可用性一、*
  ...
  - **核心机制**（审查时必须理解）：
    ...
    - 跨进、跨模块场景下，子进程已检测成功的状态变更必须通过 SSE / 状态广播实时同步到前端*禁止**前端30 TTL 缓存兜底（子进程状态变更后 30 秒内前端仍显示旧状态）
...
- 🆕v4.11【强制、*F-REVIEW-STATE-ENUM-ALIGN：前后端状态枚举值对、*
  ...
  - **适用**：所有有状态字段的业务对象（任、会话/订单/评估）；后端Enum 或常量定义状态值的场景
...
- 🆕v4.6【强制、*F-REVIEW-MULTI-WRITE-ENTRY-FRONTEND：多源状态同步统一入口（前端侧、*
  ...
  - **修复模式**（统一 refetch 入口 + 集中状态管理）    ```typescript
    ...
    const handleControl = async (action) => {
      ...
      await refetch(taskId)  // ͳһ refetch
    }
    // SSE 推送路、    useEffect(() => {
...
- 🆕v4.14【强制、*F-REVIEW-FILTER-BACKEND-ALIGN：前端过滤与后端分类对齐**
  ...
  - **判断信号**    - 前端filterStatus 状态但，grep 后端分类逻辑 、视为违规
  ...
  - **核心机制**（审查时必须理解）：
    - `total=filteredItems.length`：分页总数基于过滤后的数据、    - `current=1`：切换过滤条件时重置到第一页，不触发后，reload（前端过滤已加载数据、    - `pageSize=filteredItems.length||1`：全量显示避免截断（过滤后数据量通常较少、  - **判断信号**    - 前端filterStatus pagination 仍用，items.length 、视为违规
...
- 🆕v4.21【强制、*F-REVIEW-CONFIG-PERSIST-VERIFY：配置持久化端到端验证规、*
  ...
  - 前端展示 LLM 模型能力（vision_capable / function_call / json_mode）时必须**集中展示**（从后端 `/api/config` `/api/about` 统一获取，禁止在每个展示页独，fetch / 内联判断）；用户上传图片 / 启用 tool 时前端必须根据能力位给出**降级状态可视化**（如 `Tag color="warning"` 显示"纯文本模型不支持图片分析，已降级为文字描、），禁止静默丢弃用户的可选输、  - **核心机制**（审查时必须理解）：
    - 后端 `api_ai._is_vision_capable()` 共享函数 + `api_ai_deep.py` 调用是后端服务端能力-需、匹配
...
- 【强制】Dev 命令：`npm run dev`（HMR，端5173- 【强制】测试命令：`npx vitest run`
...
- 🆕v4.7【强制、*F-REVIEW-REUSE-PATTERN-FRONTEND：复用既有模式原则（前端侧）**
  ...
  - **关键约束**    - 新增 Hook 前必，grep `use*.ts` 确认无相似实、    - 新增工具函数前必，grep `utils/` / `helpers/` 确认无相似实、    - 强行复用导致耦合 > 重新实现的成本时，允许重新实现但需注释说明
    - 复用 Ant Design 组件时必须确认版本兼容性（项目AntD 5.21  - **适用**：新增前端功能（Hook/组件/工具函数/模式）前的预检、  - **不适用**：首次实现的基础设施代码（无既有实现可复用）、业务逻辑差异较大的场景（强行复用会导致耦合、  - **历史教训**：评估明细采集按钮未复用项目内既有的 `message.loading` + `try/catch` + `message.success/error` 三态反馈模式，导致无用户反馈。修复后复用既有模式实现三态反、- 🆕v4.19【强制、*F-REVIEW-SCHEDULER-STATUS-DISPLAY：调度器状态展示规、*
...
基于 2026-07-05 完成，MU2 Sprint（认证中间件改、+ CookieStore 扩展 user_id 维度）复盘（使用 Sequential Thinking 8 步复盘法——成功步、不确定性与失败、可抽象的固定流程与判断逻辑/适用场景与不适用场景），系统化梳理前端在多用户认证场景下的上下文隔离原则。本维度强调"前端user_id 隔离状、+ 认证 token 通过 httpOnly cookie 传、+ 401 降级路径明确分离"，确保多用户场景下不发生跨用户污染、认证失败有可操作的恢复路径、 项检查点对应 `xianyu-hunter-dev` 编码规范 step 134-137（多用户资源隔离 + 认证中间件多路校、+ 会话 token 安全管理 + 快照与实时数据覆盖决策）
...
## 46. UI 操作项分级保留与多视图一致性 🆕v4.52.0

> 本维度对应 `xianyu-hunter-dev` v4.49.0 meta-rule #83 落地（experimental）。新增 2 项 F-REVIEW 检查点（F-REVIEW-197/198），覆盖 step 236/237/238 三条编码规范。所有阈值参数通过 `config.yaml#ui_layout` 节点集中管理，禁止代码硬编码。

### F-REVIEW-197: ACTION-BUTTON-TIER-RETAIN 操作项分级保留与横向滚动 fallback

- **规范**：单行/单卡操作按钮总数 > `ui_layout.action_button_visible_threshold`（默认 5）时，必须按使用频率分级（高频文字 / 中频图标+Tooltip / 低频 Dropdown/Popover/Drawer 收起）；所有 AntD `<Table>` 必须显式声明 `scroll={{ x: <minWidth 或 'max-content'> }}` 作为兜底
- **判断信号**：
  - `grep "<Button" <page>.tsx` 在单个 `<Space>` / `actions=` 内计数 > `ui_layout.action_button_visible_threshold`（默认 5）→ 违规
...
- **配置节点**：`config.yaml#ui_layout`
  - `enabled`：是否启用（默认 true）
  - `action_button_visible_threshold`：文字按钮可见阈值（默认 5）
  - `low_frequency_threshold_per_month`：低频判定阈值（默认 1 次/月）
  - `collapse_container_strategy`：收起容器选择策略（le3 → Dropdown / 3to6 → Popover / gt6 → Drawer）
  - `scroll_fallback`：横向滚动策略（默认 'max-content'）
- **适用场景**：列表页操作列（任务管理、订单管理、商品管理）、卡片视图操作区、详情页固定操作栏、AntD `<Table>` columns render 中承载多个操作按钮的场景
- **不适用场景**：操作按钮总数 ≤ 阈值、表单提交按钮组（主/次按钮天然分级）、详情页无溢出风险的固定操作栏、列数 ≤ 3 的简单表格
- **修复模式**：
  ```tsx
  // ✅ 正确：高频文字 + 中频图标 + 低频 Dropdown + scroll fallback
  <Table
    ...
    columns={[..., {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Button type="link" onClick={handlers.toggle}>{record.running ? '暂停' : '启动'}</Button>
          <Button type="link" onClick={handlers.stop}>停止</Button>
          <Button type="link" icon={<CopyOutlined />} onClick={handlers.copy}><Tooltip title="复制" /></Button>
          <Button type="link" danger onClick={handlers.delete}>删除</Button>
  ...
  />
  ```
- **历史教训**：TaskList.tsx 操作列 6 个文字按钮（编辑/启停/停止/复制/另存为模板/删除）在默认列宽下宽度约 580px 超出页面样式边界。修复：将"另存为模板"收入 Dropdown 更多菜单（操作列宽度降到约 380px），Table 显式声明 `scroll={{ x: 'max-content' }}` 作为兜底
- **对应规范**：`xianyu-hunter-dev` step 236（操作项分级保留规范）+ step 238（UI 水平空间溢出诊断与横向滚动容错规范）

### F-REVIEW-198: MULTI-VIEW-MODE-CONSISTENCY 多视图模式一致性

- **规范**：同一资源在表格视图 + 卡片视图双视图呈现时，操作按钮集合必须**完全一致**（按钮数量、文案、handler 行为、disabled 条件），禁止分叉实现
- **判断信号**：
  ...
  - 双视图的 disabled 条件不一致（如表格视图禁用但卡片视图可点击）→ 违规
- **配置节点**：`config.yaml#ui_layout`
  - `multi_view_sync_required`：多视图同步要求（默认 true）
  - `multi_view_sync_scan_glob`：扫描的文件 glob（默认 `frontend/src/pages/**/*.tsx`）
  - `table_view_matchers`：识别表格视图的特征字符串（默认 `["<Table", "columns=", "scroll="]`）
  - `card_view_matchers`：识别卡片视图的特征字符串（默认 `["<Card", "actions=", "cover="]`）
- **适用场景**：同时存在表格视图 + 卡片视图的列表页（任务管理、订单管理）、同时存在列表视图 + 网格视图的页面、同时存在详细视图 + 简略视图的页面
- **不适用场景**：仅有一种视图的页面、双视图但操作集合本身不同（如管理员视图 vs 普通用户视图，操作集合本就不同）
- **修复模式**：
  ```tsx

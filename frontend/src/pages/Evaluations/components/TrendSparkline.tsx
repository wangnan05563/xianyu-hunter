      ...
      <Row gutter={16} align="middle">
        <Col span={6}>
          ...
          <div style={{ marginTop: 4 }}>
            <Badge status={(() => {
              // 涨=风险(error) 跌=利好(success) 平=default
              if (trend.trend === 'up') return 'error'
              if (trend.trend === 'down') return 'success'
              return 'default'
            })()} text={(() => {
              if (trend.trend === 'up') return '涨价 ↑'
              if (trend.trend === 'down') return '降价 ↓'
              return '稳定 →'
            })()} />

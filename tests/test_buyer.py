...
class FakeBrowser:
    ...
    def __init__(self, scenario: _BuyScenario):
        ...
        self.external_pages: set[FakePage] = set()
    ...
    def register_external_page(self, page: FakePage) -> None:
        self.register_calls += 1
        self.external_pages.add(page)

    def unregister_external_page(self, page: FakePage) -> None:
        self.unregister_calls += 1
        self.external_pages.discard(page)
...
@pytest.mark.asyncio
async def test_buy_own_page_registers_external_page() -> None:
    """Buyer-created pages are protected from scheduler cleanup."""
    scenario = _BuyScenario(actual_price=1999.0)
    buyer, _, browser, _ = make_buyer(scenario)

    result = await buyer.buy(task_id="t1", item_id="i1", expected_price=1999.0)

    assert result.outcome.value == "success"
    assert browser.register_calls == 1
    assert browser.unregister_calls == 1
    assert browser.external_pages == set()

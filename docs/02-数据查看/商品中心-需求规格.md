...
- 下一阶段智能体：主智能体（已并行启动）
- 交接上下文：
  · 本 SRS 与详细设计 v2.0 同步，14 个 API 路径/字段/默认值与 src/xianyu_hunter/web/routes/api_items.py、api_task_links.py、api_export.py 完全一致；
...
| 认证白名单（`/api/auth/cookie` 等） | 不适用 | 全部 14 个 API 需通过认证（继承现有 [auth.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/middleware/auth.py) 中间件） |
...
| 后端 API 路由 | [src/xianyu_hunter/web/routes/api_items.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/routes/api_items.py) / [api_task_links.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/routes/api_task_links.py) / [api_export.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/routes/api_export.py) | 14 个路由实现 |
| ItemRow 模型 | [db_models.py#L98-L131](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/infra/db_models.py#L98-L131) | 18 字段 |
| TaskLinkRow 模型 | [db_models.py#L256-L280](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/infra/db_models.py#L256-L280) | 9 字段 |
...
| 依赖注入容器 | [src/xianyu_hunter/container.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/container.py) | `container.repo` 注入 |
| 实时搜索服务 | [search_services.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/services/search_services.py) | live 链路核心服务 |
| 采集服务 | [collection_service.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/modules/collection_service.py) | `collector.detail` 调用方 |
| 采集工具 | [collector_utils.py#L545-L556](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/modules/collector_utils.py#L545-L556) | FIELD_METADATA 字段映射 |
...
2. **复用现有认证**：复用 [auth.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/middleware/auth.py) 中间件
...
- 依赖现有 [collector.detail](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/modules/collection_service.py) 采集商品详情
- 依赖现有 [browser_lock](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/modules/collection_service.py) 浏览器锁（live 链路需获取）
- 依赖现有 [evaluator.evaluate](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/modules/evaluator.py) 评估器（live 链路复用配置）
...
定义于 [db_models.py#L98-L131](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/infra/db_models.py#L98-L131)，共 18 字段：
...
定义于 [db_models.py#L256-L280](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/infra/db_models.py#L256-L280)，共 9 字段：
...
- **SR-1**：所有 14 个 `/api/items/*` / `/api/tasks/{tid}/links/*` / `/api/export/*` 接口必须通过现有 [auth.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/middleware/auth.py) 中间件认证
...
- **SR-13**：`container` 内部单例在进程启动时初始化（[startup.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/startup.py)），避免循环依赖
...
- **后端路由**：
  - 商品概要：[src/xianyu_hunter/web/routes/api_items.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/routes/api_items.py)（3 个路由）
  - 任务关联：[src/xianyu_hunter/web/routes/api_task_links.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/routes/api_task_links.py)（9 个路由）
  - 导出：[src/xianyu_hunter/web/routes/api_export.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/routes/api_export.py)（2 个路由）
- **服务层**：[src/xianyu_hunter/web/services/search_services.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/services/search_services.py)（实时搜索核心：60s 缓存、in-flight 去重、身份校验、SSE 推送、30s 抢单冷却）
- **数据模型**：[src/xianyu_hunter/infra/db_models.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/infra/db_models.py) L98-L131（ItemRow 18 字段）/ L256-L280（TaskLinkRow 9 字段）
...
**后端**：三个路由文件在 [startup.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/startup.py) 中通过 `app.include_router(api_items.router)` / `app.include_router(api_task_links.router)` / `app.include_router(api_export.router)` 注册（已集成）。
...
**关联代码**：
- 商品概要路由：[src/xianyu_hunter/web/routes/api_items.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/routes/api_items.py)
- 任务关联路由：[src/xianyu_hunter/web/routes/api_task_links.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/routes/api_task_links.py)
- 导出路由：[src/xianyu_hunter/web/routes/api_export.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/routes/api_export.py)
- 实时搜索服务：[src/xianyu_hunter/web/services/search_services.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/services/search_services.py)
...
- 采集服务：[src/xianyu_hunter/modules/collection_service.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/modules/collection_service.py)
- 采集工具：[src/xianyu_hunter/modules/collector_utils.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/modules/collector_utils.py) L545-L556

...
所有端点定义于 `backend/xianyu_hunter/web/routes/api_logs.py`，router prefix=`/api/logs`，tags=`logs`。
...
定义于 `backend/xianyu_hunter/infra/request_context.py`。
...
定义于 `backend/xianyu_hunter/web/middleware/error_capture.py`。两个入口均写入 `error_logs` 表（由"错误日志"文档覆盖字段细节），本节仅澄清捕获机制。
...
| 错误捕获中间件      | `backend/xianyu_hunter/web/middleware/error_capture.py`                                            | 写入 error_logs 表（全链路追踪端点读取）          |
| 事件时间线 SSE      | `backend/xianyu_hunter/web/routes/sse_stream.py`                                                   | 前端实时日志页实际订阅的 SSE 通道                 |
| Repository          | `backend/xianyu_hunter/infra/repo_events.py`                                                       | `list_events`/`list_events_by_request`/`max_event_id` 等查询 |
| DB 模型             | `backend/xianyu_hunter/infra/db_models.py`                                                         | EventRow 定义（第 194 行）                        |

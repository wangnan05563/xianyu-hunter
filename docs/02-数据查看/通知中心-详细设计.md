...
所有端点位于 `src/xianyu_hunter/web/routes/api_notifications.py`，router 前缀 `/api/notifications`，tags=`notifications`。共 **7 个端点**。
...
文件位置：`src/xianyu_hunter/web/services/notification_engine.py`。
...
**调用方**：
- `src/xianyu_hunter/web/routes/auth_query.py:223` 在 `/api/auth/me` 端点中调用，传入 `auth_state=result`；
...
| 依赖项                  | 文件路径                                                              | 说明                                                                 |
| ----------------------- | --------------------------------------------------------------------- | -------------------------------------------------------------------- |
| NotificationRow 数据模型| `src/xianyu_hunter/infra/db_models.py:305`                            | 9 字段定义 + `uq_notification_dedup` 唯一约束                        |
| 通知仓储层              | `src/xianyu_hunter/infra/repo_notifications.py`                       | `NotificationsMixin` 提供 8 个 CRUD 方法，混入主 Repo                |
| 通知规则引擎            | `src/xianyu_hunter/web/services/notification_engine.py`               | `scan_and_notify` 聚合入口，含订单超时与登录态掉线两条规则           |
| 通知 API 路由           | `src/xianyu_hunter/web/routes/api_notifications.py`                   | 7 个端点定义                                                          |
| Container 依赖注入      | `src/xianyu_hunter/container.py`                                      | 通过 `get_container` 注入 `repo`                                     |

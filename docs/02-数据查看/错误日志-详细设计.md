...
所有端点定义于 `src/xianyu_hunter/web/routes/api_error_logs.py`，router prefix=`/api/error-logs`，tags=`error-logs`。
...
定义于 `src/xianyu_hunter/web/middleware/error_capture.py`。两个入口均写入 `error_logs` 表，统一调用 `_save_error_log` 内部函数。
...
| 模块                | 文件路径                                                                                       | 依赖关系                                          |
| ------------------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| 错误捕获中间件      | `src/xianyu_hunter/web/middleware/error_capture.py`                                            | 写入 error_logs 表的两个入口                      |
| 异常处理器          | `src/xianyu_hunter/web/middleware/exception_handler.py`                                        | 调用 `capture_request_error`                      |
| request_id 生成器   | `src/xianyu_hunter/infra/request_context.py`                                                   | 提供 request_id 生成、ContextVar 传递、格式校验   |
| Repository          | `src/xianyu_hunter/infra/repo_error_logs.py`                                                   | `ErrorLogsMixin` 提供 CRUD + 批量 + 统计 + 清理  |
| 搜索服务            | `src/xianyu_hunter/web/services/search_services.py`                                            | `ErrorLogSearchService` 处理分页与慢查询埋点      |
| DB 模型             | `src/xianyu_hunter/infra/db_models.py`                                                         | `ErrorLogRow` 定义（第 219 行）                   |

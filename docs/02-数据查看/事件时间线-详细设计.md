...
### 5.1 events 表（EventRow，`src/xianyu_hunter/infra/db_models.py:194`）
...
来源：`src/xianyu_hunter/domain/events.py:9` `EventType(str, Enum)`，共 **35 种**。
...
| 方法 | 路径                | 文件                              | 用途                              |
| ---- | ------------------- | --------------------------------- | --------------------------------- |
| GET  | `/api/timeline`     | `src/xianyu_hunter/web/routes/timeline.py`     | 时间线聚合 API（orders + events） |
| GET  | `/api/events/stream`| `src/xianyu_hunter/web/routes/sse_stream.py`   | SSE 实时事件流（含断线回放）      |
...
来源：`src/xianyu_hunter/infra/event_bus.py`
...
来源：`src/xianyu_hunter/infra/request_context.py`
...
| 依赖                          | 位置                                            | 说明                                                       |
| ----------------------------- | ----------------------------------------------- | ---------------------------------------------------------- |
| `EventType` 枚举              | `src/xianyu_hunter/domain/events.py`            | 35 种事件类型定义与严重度                                  |
| `Event` dataclass             | `src/xianyu_hunter/domain/events.py:115`        | EventBus 传递的最小单元（type/task_id/item_id/payload/timestamp/severity/request_id）|
| `container.repo.list_orders`  | `src/xianyu_hunter/infra/repo_orders.py`        | 时间线 orders 来源                                         |
| `container.repo.list_events`  | `src/xianyu_hunter/infra/repo_events.py`        | 时间线 events 来源；SSE 增量与回放来源                     |
| `container.repo.max_event_id` | `src/xianyu_hunter/infra/repo_events.py`        | SSE 增量轮询的游标                                         |

...
| 任务 CRUD 后端 | [../../backend/xianyu_hunter/web/routes/api_tasks.py](../../backend/xianyu_hunter/web/routes/api_tasks.py) | 任务 CRUD + 控制 + 批量 7 端点 |
| 任务依赖后端 | [../../backend/xianyu_hunter/web/routes/api_task_deps.py](../../backend/xianyu_hunter/web/routes/api_task_deps.py) | 任务依赖 4 端点 |
| 任务关联后端 | [../../backend/xianyu_hunter/web/routes/api_task_links.py](../../backend/xianyu_hunter/web/routes/api_task_links.py) | 任务-内容关联 7 端点 + SSE |
| 领域模型 | [../../backend/xianyu_hunter/domain/task.py](../../backend/xianyu_hunter/domain/task.py) | TaskMode/TaskStatus/Task/TaskConfig + XIANYU_FILTER_MAP |
| 调度器 | [../../backend/xianyu_hunter/modules/scheduler.py](../../backend/xianyu_hunter/modules/scheduler.py) | TaskScheduler + F-16 依赖激活 + _RESUME_COOLDOWN |
| DB 模型 | [../../backend/xianyu_hunter/infra/db_models.py](../../backend/xianyu_hunter/infra/db_models.py) | TaskRow/TaskLinkRow/TaskDepRow |
| 仓储 | [../../backend/xianyu_hunter/infra/repo_tasks.py](../../backend/xianyu_hunter/infra/repo_tasks.py) | 任务仓储 |
| 运行记录聚合 | [../../backend/xianyu_hunter/infra/repo_events.py](../../backend/xianyu_hunter/infra/repo_events.py) | `get_task_runs` |
...
1. **集成部署**：作为 `backend/xianyu_hunter/web/routes/api_*.py` 路由 + `frontend/src/pages/Tasks/*` 页面集成到现有系统，不独立部署
...
| 模块 | 文件 | 关键行/函数 |
|------|------|-------------|
| 任务 CRUD + 控制 + 批量 | `backend/xianyu_hunter/web/routes/api_tasks.py` | `TaskCreate` / `TaskUpdate` / `list_tasks` / `create_task` / `get_task` / `update_task` / `delete_task` / `control_task` / `batch_control_tasks` / `get_task_runs` |
| 任务依赖 | `backend/xianyu_hunter/web/routes/api_task_deps.py` | `add_dep` / `remove_dep` / `list_deps` / `list_dependents` / `check_circular_dep` |
| 任务-内容关联 + SSE | `backend/xianyu_hunter/web/routes/api_task_links.py` | `list_links` / `count_links` / `create_link` / `delete_link` / `refresh_links` / `live_links_sse` / `lookup_link` / `search_links` / `auto_migrate_links` |
| 智能建任务 | `backend/xianyu_hunter/web/routes/api_ai.py` | `parse_task` |
| 模板市场 | `backend/xianyu_hunter/web/routes/api_templates.py` | `list_templates` / `create_template` / `delete_template` |
| Cron 校验 | `backend/xianyu_hunter/web/routes/api_cron.py` | `validate_cron` / `cron_examples` |
| 领域模型 | `backend/xianyu_hunter/domain/task.py` | `TaskMode` / `TaskStatus` / `Task` / `TaskConfig` / `XIANYU_FILTER_MAP` |
| 调度器 | `backend/xianyu_hunter/modules/scheduler.py` | `TaskScheduler` / `TaskWorker` / `trigger_dependent_tasks` / `_check_resume_allowed` / `_pause_if_exceeded_fail_threshold` / `_RESUME_COOLDOWN_SECONDS` / `_run_lock` |
| 任务仓储 | `backend/xianyu_hunter/infra/repo_tasks.py` | `list_tasks` / `get_task` / `upsert_task` / `delete_task_cascade` / `add_task_dep` / `remove_task_dep` / `list_task_deps` / `list_dependent_tasks` / `check_circular_dep` / `update_task_status` |
| 运行记录聚合 | `backend/xianyu_hunter/infra/repo_events.py` | `get_task_runs`（5 分钟 bucket + idle_gaps） |
| DB 模型 | `backend/xianyu_hunter/infra/db_models.py` | `TaskRow`（23 字段） / `TaskLinkRow`（9 字段） / `TaskDepRow`（4 字段） |
| CookieStore | `backend/xianyu_hunter/web/services/cookie_store.py` | `CookieStore` / `has_valid_cookies` / `is_test_cookie` / `invalidate_cache` |

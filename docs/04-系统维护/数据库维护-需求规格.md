...
- **不实现 DDL（建表 / 删表 / 改字段）**：表结构由 SQLAlchemy ORM 集中管理（`init_db` 自动迁移），如需调整表结构应改后端 ORM 定义（`src/xianyu_hunter/infra/db_models.py`）后重启
...
| DB 索引策略（`task_id` / `seller_id` / `first_seen` / `publish_time` / `created_at` 及复合索引） | 适用 | 11 张业务表继承 [db_models.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/infra/db_models.py) 的索引策略 |
...
| SearchService | 复用 [search_services.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/services/search_services.py) 的 `DbAdminSearchService`，统一慢查询埋点 |
...
| 后端路由 | [src/xianyu_hunter/web/routes/api_db_admin.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/routes/api_db_admin.py) | 11 个 API 端点 + 工具函数 |
| 搜索 Service | [src/xianyu_hunter/web/services/search_services.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/services/search_services.py) | `DbAdminSearchService` 复用 |
| ORM 模型 | [src/xianyu_hunter/infra/db_models.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/infra/db_models.py) | 11 张业务表 DDL 与索引 |
...
1. **集成部署**：作为 `src/xianyu_hunter/web/routes/api_db_admin.py` 路由文件集成到现有后端，不独立部署
2. **复用 ORM**：表结构由 [db_models.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/infra/db_models.py) 统一管理，本模块不引入 DDL 能力
3. **复用 SearchService**：行级查询委托 `DbAdminSearchService`（[search_services.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/services/search_services.py)），保持慢查询埋点一致
4. **复用认证体系**：复用现有 [auth.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/middleware/auth.py) 中间件
...
**假设**：
- 用户已通过现有 Cookie 认证（[auth.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/middleware/auth.py)）登录
...
- 依赖现有 [container.repo](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/container.py) 提供 `engine` / `save_event`
- 依赖 [search_base.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/services/search_base.py) 的 `SearchParams` / `SearchService` 基类
...
**FR-2.4 不做 DDL**
- 本模块不提供建表/删表/改字段能力
- 表结构变更需改 [db_models.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/infra/db_models.py) 后重启（`init_db` 自动迁移）
...
11 张业务表与 [db_models.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/infra/db_models.py) 一一对应：
...
> 注：表 DDL / 索引策略见 [db_models.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/infra/db_models.py)，本模块不重复定义。
...
events 表结构详见 [db_models.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/infra/db_models.py) §5.6。本模块写入约定：
...
- **表结构**：11 张业务表由 [db_models.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/infra/db_models.py) 定义
- **索引**：表 schema 已在 [db_models.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/infra/db_models.py) 中声明（含 `task_id` / `seller_id` / `first_seen` / `publish_time` / `created_at` 及复合索引）
...
**设计要点**：
- 与 [db_models.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/infra/db_models.py) 中 `__tablename__` 一一对应
...
- `DbAdminSearchService` 位于 [search_services.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/services/search_services.py) L131
- 复用 `SearchService` 基类（[search_base.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/services/search_base.py)）的：
  - 分页契约
  - 慢查询埋点
  - 响应结构
...
| 后端路由 | [api_db_admin.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/routes/api_db_admin.py) | L1-940 |
| 搜索 Service | [search_services.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/web/services/search_services.py) | L117-211 `DbAdminSearchService` |
| ORM 模型 | [db_models.py](file:///d:/code/otherProjects/17_xianyu/src/xianyu_hunter/infra/db_models.py) | 11 张业务表 DDL |

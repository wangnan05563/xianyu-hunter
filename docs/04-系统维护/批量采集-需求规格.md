...
| [src/xianyu_hunter/web/routes/api_batch_refresh.py](../../src/xianyu_hunter/web/routes/api_batch_refresh.py) | 11 个 REST 端点实现 |
| [src/xianyu_hunter/modules/batch_refresh_scheduler.py](../../src/xianyu_hunter/modules/batch_refresh_scheduler.py) | 调度器主类与流程拆分 |
| [src/xianyu_hunter/infra/repo_batch_refresh_history.py](../../src/xianyu_hunter/infra/repo_batch_refresh_history.py) | 历史仓储（`BatchRefreshHistoryMixin`） |
| [src/xianyu_hunter/modules/collection_service.py](../../src/xianyu_hunter/modules/collection_service.py) | 统一采集服务（`ItemCollectionService.collect()`） |
...
| 数据库 | SQLite（复用现有 [db_models.py](../../src/xianyu_hunter/infra/db_models.py) §5.20/§5.21） |
| 启动参数 | `XH_WITH_SCHEDULER=1` 启用调度器；`batch_refresh.enabled=true` 注册 job |

### 2.4 设计约束

1. **APScheduler 集成**：参照 [cookie_sync_scheduler.py](../../src/xianyu_hunter/modules/cookie_sync_scheduler.py) 的 `BackgroundScheduler` 模式（项目已有先例）
...
**依赖**：
- 依赖现有 [container.py](../../src/xianyu_hunter/container.py) 依赖注入容器
- 依赖现有 [config.py](../../src/xianyu_hunter/config.py) 配置加载机制
- 依赖现有 [db_models.py](../../src/xianyu_hunter/infra/db_models.py) §5.20/§5.21 表结构
- 依赖现有 [collection_service.py](../../src/xianyu_hunter/modules/collection_service.py) 统一采集
- 依赖现有 [auth.py](../../src/xianyu_hunter/web/middleware/auth.py) 中间件
- 依赖现有 [cookie_runtime_sync.py](../../src/xianyu_hunter/web/services/cookie_runtime_sync.py) Cookie 同步
...
定义在 [yaml_config.py](../../src/xianyu_hunter/infra/yaml_config.py) §BatchRefreshConfig：
...
定义在 [db_models.py](../../src/xianyu_hunter/infra/db_models.py) §5.20。同批次运行仅一条记录，批次结束清理。
...
定义在 [db_models.py](../../src/xianyu_hunter/infra/db_models.py) §5.21。持久化每次执行的完整记录。
...
- 参照 [cookie_sync_scheduler.py](../../src/xianyu_hunter/modules/cookie_sync_scheduler.py) 项目先例
...
- 走 [cookie_runtime_sync.py](../../src/xianyu_hunter/web/services/cookie_runtime_sync.py) `inject_cookie_store_to_worker_browser`
...
| 数据模型 | [db_models.py](../../src/xianyu_hunter/infra/db_models.py) §5.20/§5.21 |
| 统一采集 | [collection_service.py](../../src/xianyu_hunter/modules/collection_service.py) |
| 调度器先例 | [cookie_sync_scheduler.py](../../src/xianyu_hunter/modules/cookie_sync_scheduler.py) |
| Cookie 同步 | [cookie_runtime_sync.py](../../src/xianyu_hunter/web/services/cookie_runtime_sync.py) |

...
| 配置 API 后端 | [../../src/xianyu_hunter/web/routes/api_config.py](../../src/xianyu_hunter/web/routes/api_config.py) | 配置 CRUD + 备份 + 回滚 + 导入/导出 + 分享（11 端点） |
| 关于 API 后端 | [../../src/xianyu_hunter/web/routes/api_about.py](../../src/xianyu_hunter/web/routes/api_about.py) | 系统元信息 + 检查更新（2 端点） |
| 配置模型 | [../../src/xianyu_hunter/infra/yaml_config.py](../../src/xianyu_hunter/infra/yaml_config.py) | AppConfig 26 字段定义 |
...
| 智能客服配置 API | [../../src/xianyu_hunter/web/routes/api_chatbot_config.py](../../src/xianyu_hunter/web/routes/api_chatbot_config.py) | `_UPDATABLE_KEYS`（不属于本模块，仅参考） |
...
| 构建信息 | `src/xianyu_hunter/_build_info.py`（由 `scripts/build_info.py` 写入） |

### 2.4 设计约束

1. **集成部署**：作为 `src/xianyu_hunter/web/routes/api_config.py` 路由 + `frontend/src/pages/Config/VersionManager.tsx` 页面集成到现有系统，不独立部署
...
- `src/xianyu_hunter/_build_info.py` 已由 `scripts/build_info.py` 在构建时写入
...
`scripts/build_info.py` 在构建时写入 `src/xianyu_hunter/_build_info.py`：
...
| 模块 | 文件 | 关键行/函数 |
|------|------|-------------|
| 配置 API | `src/xianyu_hunter/web/routes/api_config.py` | `BACKUP_KEEP` / `BACKUP_PREFIX` / `CONFIG_FILE` / `BACKUP_DIR` / `_new_backup_name` / `_list_backups` / `_prune_backups` / `_resolve_backup` / `_safe_app_version` / `save_config` / `list_backups` / `_SHARE_REDACT_PATHS` / `_redact_for_share` / `share_config`（僵尸）/ `export_config` / `import_config` / `get_config_version`（僵尸）/ `rollback_config` / `restore_backup` / `_SENSITIVE_KEYS` / `_sanitize_dict` |
| 关于 API | `src/xianyu_hunter/web/routes/api_about.py` | `_safe_build_info` / `get_about` / `check_update` |
| 配置模型 | `src/xianyu_hunter/infra/yaml_config.py` | `AppConfig`（26 字段） / `reload_config` |
| 构建信息 | `src/xianyu_hunter/_build_info.py` | `__version__` / `build_date` / `git_sha` |

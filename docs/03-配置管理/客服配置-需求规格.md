...
| 配置 API 后端 | [../../src/xianyu_hunter/web/routes/api_chatbot_config.py](../../src/xianyu_hunter/web/routes/api_chatbot_config.py) | 配置/审计/FAQ/welcome 接口 |
| 对话 API 后端 | [../../src/xianyu_hunter/web/routes/api_chatbot.py](../../src/xianyu_hunter/web/routes/api_chatbot.py) | 14 个对话与会话端点 |
| KB API 后端 | [../../src/xianyu_hunter/web/routes/api_kb.py](../../src/xianyu_hunter/web/routes/api_kb.py) | 知识库 4 端点 |
| 向量库维护 API | [../../src/xianyu_hunter/web/routes/api_vector_admin.py](../../src/xianyu_hunter/web/routes/api_vector_admin.py) | vector-admin Tab 后端 |
| Pydantic 模型 | [../../src/xianyu_hunter/infra/yaml_config.py](../../src/xianyu_hunter/infra/yaml_config.py) | ChatbotConfig 9 子模型 |
| DB 模型 | [../../src/xianyu_hunter/infra/db_models.py](../../src/xianyu_hunter/infra/db_models.py) | 7 张表 schema |
| 编排器 | [../../src/xianyu_hunter/modules/chatbot/orchestrator.py](../../src/xianyu_hunter/modules/chatbot/orchestrator.py) | ChatbotOrchestrator + per-session Lock |
| Agent | [../../src/xianyu_hunter/modules/chatbot/agent.py](../../src/xianyu_hunter/modules/chatbot/agent.py) | Agent 类（OpenAI function_calling） |
| Escalation | [../../src/xianyu_hunter/modules/chatbot/escalation.py](../../src/xianyu_hunter/modules/chatbot/escalation.py) | 转人工判定（5 触发条件） |
| KBManager | [../../src/xianyu_hunter/modules/chatbot/kb_manager.py](../../src/xianyu_hunter/modules/chatbot/kb_manager.py) | 两阶段提交 + 快照恢复 |
| EmbeddingService | [../../src/xianyu_hunter/modules/chatbot/embedding_service.py](../../src/xianyu_hunter/modules/chatbot/embedding_service.py) | 后端选择 |
| 本地 Embedding | [../../src/xianyu_hunter/modules/chatbot/local_embedding.py](../../src/xianyu_hunter/modules/chatbot/local_embedding.py) | sentence-transformers + HF 镜像 |
| 工具注册表 | [../../src/xianyu_hunter/modules/chatbot/tool_registry.py](../../src/xianyu_hunter/modules/chatbot/tool_registry.py) | 5 个默认工具 |
...
1. **集成部署**：作为 `src/xianyu_hunter/web/routes/api_chatbot*.py` / `api_kb.py` 路由 + `frontend/src/pages/Chatbot/Config.tsx` 页面集成到现有系统
...
```json
{
  ...
  "kb": {"auto_update_enabled": true, "update_interval_hours": 6, "doc_paths": ["docs/", "src/xianyu_hunter/"], "embedding_concurrency": 5, "embedding_model": "text-embedding-3-small", "embedding_dimensions": 1536, "chunk_size": 500, "chunk_overlap": 50, "snapshot_max_keep": 10, "persist_path": "data/chromadb", "collection_name": "xianyu_hunter_docs", "project_root": "."},
...
| 模块 | 文件 | 关键行/函数 |
|------|------|-------------|
| 配置与审计 | `src/xianyu_hunter/web/routes/api_chatbot_config.py` | `_UPDATABLE_KEYS` / `_DB_OVERRIDE_RULES` / `get_config` / `update_config` / `list_audit_logs` / `get_welcome` / FAQ CRUD |
| 对话与会话 | `src/xianyu_hunter/web/routes/api_chatbot.py` | `chat` SSE / 会话 CRUD 7 / 消息历史 / 撤回 / 反馈 / 转人工 / 导出 |
| 知识库 | `src/xianyu_hunter/web/routes/api_kb.py` | `rebuild_kb` / `kb_status` / `list_kb_versions` / `rollback_kb`（路径参数） |
| 向量库维护 | `src/xianyu_hunter/web/routes/api_vector_admin.py` | 向量库维护端点 |
| Pydantic 模型 | `src/xianyu_hunter/infra/yaml_config.py` | `ChatbotConfig` / `ChatbotRAGConfig`(5) / `ChatbotLLMConfig`(6) / `ChatbotAgentConfig`(6) / `ChatbotKBConfig`(12) / `ChatbotFAQConfig`(2) / `ChatbotEscalationConfig`(4) |
| DB 模型 | `src/xianyu_hunter/infra/db_models.py` | 7 张表（sessions/messages/faqs/feedback/kb_versions/config/audit_logs） |
| 仓储 | `src/xianyu_hunter/infra/repo_chatbot.py` | `ChatbotRepository` |
| 编排器 | `src/xianyu_hunter/modules/chatbot/orchestrator.py` | `ChatbotOrchestrator` + per-session Lock + 降级链 |
| Agent | `src/xianyu_hunter/modules/chatbot/agent.py` | `Agent` 类（OpenAI function_calling） |
| RAG 引擎 | `src/xianyu_hunter/modules/chatbot/rag_engine.py` | `RAGEngine` |
| KB 管理器 | `src/xianyu_hunter/modules/chatbot/kb_manager.py` | `KBManager` 两阶段提交 + 快照恢复 |
| 转人工 | `src/xianyu_hunter/modules/chatbot/escalation.py` | `Escalation` 5 触发条件 |
| Embedding 服务 | `src/xianyu_hunter/modules/chatbot/embedding_service.py` | `EmbeddingService` 后端选择 |
| 本地 Embedding | `src/xianyu_hunter/modules/chatbot/local_embedding.py` | sentence-transformers + HF 镜像 |
| 工具注册表 | `src/xianyu_hunter/modules/chatbot/tool_registry.py` | `ToolRegistry` + 5 工具注册 |
| 工具实现 | `src/xianyu_hunter/modules/chatbot/tools/*.py` | 5 个工具类 |
| FAQ 匹配器 | `src/xianyu_hunter/modules/chatbot/faq_matcher.py` | `FAQMatcher` 5 分钟 TTL |
| PII 脱敏 | `src/xianyu_hunter/modules/chatbot/sanitizer.py` | `sanitize_session_for_copy` |

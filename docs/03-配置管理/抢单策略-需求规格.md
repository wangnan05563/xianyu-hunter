...
| 通用配置后端 | [../../backend/xianyu_hunter/web/routes/api_config.py](../../backend/xianyu_hunter/web/routes/api_config.py) | 9 个通用配置端点 |
| 抢单订单后端 | [../../backend/xianyu_hunter/web/routes/api_orders.py](../../backend/xianyu_hunter/web/routes/api_orders.py) | 8 个抢单相关端点 + F-16 + manual-takeover |
| 接管超时调度器 | [../../backend/xianyu_hunter/modules/takeover_timeout_scheduler.py](../../backend/xianyu_hunter/modules/takeover_timeout_scheduler.py) | TakeoverTimeoutScheduler |
| 配置模型 | [../../backend/xianyu_hunter/infra/yaml_config.py](../../backend/xianyu_hunter/infra/yaml_config.py) | EvalConfig Pydantic 模型 |
| 抢单运行时 | [../../backend/xianyu_hunter/modules/buyer_config.py](../../backend/xianyu_hunter/modules/buyer_config.py) | BuyerConfig dataclass |
| 抢单核心 | [../../backend/xianyu_hunter/modules/buyer.py](../../backend/xianyu_hunter/modules/buyer.py) | 90 秒超时、LRU 幂等、_do_buy 流程 |
| Worker | [../../backend/xianyu_hunter/modules/worker.py](../../backend/xianyu_hunter/modules/worker.py) | _should_buy/_in_cooldown/_attempt_buy |
| 评估器 | [../../backend/xianyu_hunter/modules/evaluator.py](../../backend/xianyu_hunter/modules/evaluator.py) | _score_to_risk 风险等级映射 |
| 任务领域 | [../../backend/xianyu_hunter/domain/task.py](../../backend/xianyu_hunter/domain/task.py) | TaskMode/TaskConfig |
| 订单领域 | [../../backend/xianyu_hunter/domain/order.py](../../backend/xianyu_hunter/domain/order.py) | OrderStatus/BuyerError 异常体系 |
| 订单仓储 | [../../backend/xianyu_hunter/infra/repo_orders.py](../../backend/xianyu_hunter/infra/repo_orders.py) | expire_takeover_pending_orders |
...
| 模块 | 文件 | 关键行/函数 |
|------|------|-------------|
| 通用配置接口 | `backend/xianyu_hunter/web/routes/api_config.py` | `get_config` / `preview_config` / `save_config` / `share_config` / `export_config` / `import_config` / `list_versions` / `rollback_config` / `restore_backup` |
| 抢单订单接口 | `backend/xianyu_hunter/web/routes/api_orders.py` | `list_orders` / `get_order` / `delete_order` / `takeover` / `takeover_confirm` / `takeover_cancel` / `update_order_status` / `manual_takeover` / `_trigger_dependent_tasks` |
| 接管超时调度器 | `backend/xianyu_hunter/modules/takeover_timeout_scheduler.py` | `TakeoverTimeoutScheduler` / `_run_cleanup_job` |
| 配置模型 | `backend/xianyu_hunter/infra/yaml_config.py` | `EvalConfig` / `_check_score_order` |
| 抢单运行时 | `backend/xianyu_hunter/modules/buyer_config.py` | `BuyerConfig` dataclass |
| 抢单核心 | `backend/xianyu_hunter/modules/buyer.py` | `Buyer` / `buy` / `_do_buy` / `_detect_sold` / `_wait_for_order_page` / `_extract_order_no` / `_extract_actual_price` |
| Worker | `backend/xianyu_hunter/modules/worker.py` | `_should_buy` / `_in_cooldown` / `_attempt_buy` |
| 评估器 | `backend/xianyu_hunter/modules/evaluator.py` | `_score_to_risk` / `EvaluationThresholds.from_config` |
| 任务领域 | `backend/xianyu_hunter/domain/task.py` | `TaskMode` / `TaskConfig` |
| 订单领域 | `backend/xianyu_hunter/domain/order.py` | `OrderStatus` / `BuyOutcome` / `BuyResult` / `BuyerError` / `ButtonNotFoundError` / `ItemSoldError` / `OutOfStockError` / `PriceMismatchError` |
| 订单仓储 | `backend/xianyu_hunter/infra/repo_orders.py` | `upsert_order` / `get_order` / `list_orders` / `find_order_by_task_item` / `expire_takeover_pending_orders` |

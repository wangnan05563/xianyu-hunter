...
| 文件路径 | 职责 |
|---|---|
| `backend/xianyu_hunter/web/services/tunnel_service.py` | Cloudflare Tunnel 服务 |
| `backend/xianyu_hunter/web/routes/api_tunnel.py` | Tunnel API 端点 |
...
| 文件路径 | 改动说明 |
|---|---|
| `backend/xianyu_hunter/web/app.py` | 注册 tunnel 路由 + cookie SameSite 调整 |
| `backend/xianyu_hunter/web/middleware/auth.py` | 认证白名单新增 /api/tunnel |
...
**Files:**
- Create: `backend/xianyu_hunter/web/services/tunnel_service.py`
...
def tunnel_service():
    ...
    with patch.object(TunnelService, "_ensure_binary", return_value=Path("/fake/cloudflared")):
        return TunnelService(local_port=8001)
...
class TunnelService:
    ...
    def __init__(self, local_port: int = 8001):
...
```bash
git add backend/xianyu_hunter/web/services/tunnel_service.py tests/test_tunnel_service.py
...
**Files:**
- Create: `backend/xianyu_hunter/web/routes/api_tunnel.py`
- Modify: `backend/xianyu_hunter/web/app.py` (注册路由)
- Modify: `backend/xianyu_hunter/web/middleware/auth.py` (白名单)
...
def get_tunnel_service():
    ...
    if _tunnel_service is None:
        from xianyu_hunter.web.services.tunnel_service import TunnelService
        _tunnel_service = TunnelService(local_port=8001)
...
在 `backend/xianyu_hunter/web/app.py` 中：
...
在 `backend/xianyu_hunter/web/middleware/auth.py` 的白名单列表中添加：
```python
"/api/tunnel",
```
...
```bash
git add backend/xianyu_hunter/web/routes/api_tunnel.py backend/xianyu_hunter/web/app.py backend/xianyu_hunter/web/middleware/auth.py tests/test_api_tunnel.py
...
**Files:**
- Modify: `backend/xianyu_hunter/web/app.py:260-281` (verify_token 函数)
...
在 `backend/xianyu_hunter/web/app.py` 的 `verify_token` 函数中，修改 `set_cookie` 调用：
...
```bash
git add backend/xianyu_hunter/web/app.py tests/test_cookie_samesite.py
...
在 PC 端浏览器访问 `http://localhost:8001/app/`（桌面端），通过 API 开启隧道：

```powershell
# 调用 API 开启隧道
Invoke-RestMethod -Uri "http://localhost:8001/api/tunnel/start" -Method Post
```

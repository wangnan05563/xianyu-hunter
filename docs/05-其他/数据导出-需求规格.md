...
| 后端导出路由 | [backend/xianyu_hunter/web/routes/api_export.py](file:///d:/code/otherProjects/17_xianyu/backend/xianyu_hunter/web/routes/api_export.py) | 后端实现 |
...
| 认证中间件 | [backend/xianyu_hunter/web/middleware/auth.py](file:///d:/code/otherProjects/17_xianyu/backend/xianyu_hunter/web/middleware/auth.py) | 认证与白名单 |
| 时间工具 | [backend/xianyu_hunter/web/utils.py](file:///d:/code/otherProjects/17_xianyu/backend/xianyu_hunter/web/utils.py) | `to_datetime` / `parse_iso_datetime` |
...
1. **集成部署**：作为 `backend/xianyu_hunter/web/routes/api_export.py` 集成到现有 Web 层
...
```python
# 伪代码示意（backend/xianyu_hunter/web/routes/api_export.py）
...
| **后端路由** | `backend/xianyu_hunter/web/routes/api_export.py` |
| **后端路由注册** | `backend/xianyu_hunter/web/app.py`（第 243 行 `app.include_router(api_export.router)`） |
| **认证中间件** | `backend/xianyu_hunter/web/middleware/auth.py` |
| **时间工具** | `backend/xianyu_hunter/web/utils.py`（`to_datetime` / `parse_iso_datetime`） |
...
| **ORM 模型** | `backend/xianyu_hunter/infra/db_models.py`（`ItemRow` / `OrderRow` / `EventRow` / `EvaluationRow`） |

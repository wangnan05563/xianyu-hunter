...
> ⚠️ **已废弃**：本文档为历史规划文档，部分内容（如 check-update "首版固定返回 has_update=False"、"source: local"、BrandCard 缺少 publishedAt 字段、依赖列表为示例等）与最新代码实现不一致。当前最新实现（已接入 GitHub Releases API 真实远程检查）以 [`关于-详细设计.md`](./关于-详细设计.md) 为准，请优先参考该文档。
...
| 字段 | 来源 | 写入时机 |
|---|---|---|
| `__version__` | `backend/xianyu_hunter/__init__.py` | 手动维护（已存在 `0.1.0`） |
...
| `backend/xianyu_hunter/web/routes/api_about.py` | FastAPI 路由 | 70 |
| `backend/xianyu_hunter/_build_info.py` | 构建时生成（`scripts/build_info.py` 写入） | 10 |
...
| `backend/xianyu_hunter/web/app.py` | 1 行 `app.include_router(api_about.router)` + Swagger UI 顶栏改 1 处链接 |
| `backend/xianyu_hunter/web/middleware/auth.py` | `WHITELIST` 增 2 项 |
...
```python
# backend/xianyu_hunter/web/routes/api_about.py
...
**修改点**：`backend/xianyu_hunter/web/middleware/auth.py` 第 32 行附近的 `WHITELIST` 集合：
...
**修改点**：`backend/xianyu_hunter/web/app.py` 第 355 行：
...
详见 [plans/关于-实施计划.md](./plans/关于-实施计划.md)。
...
| 用途 | 路径 |
|---|---|
| 需求规格 | [关于-需求规格.md](./关于-需求规格.md) |
| 本设计 | `docs/关于-概要设计.md` |
| 实施计划 | [plans/关于-实施计划.md](./plans/关于-实施计划.md) |
| 视觉规范 | [michelin-design-system.md](./michelin-design-system.md) |
| 目录规范 | [目录结构规范.md](./目录结构规范.md) |
| MainLayout | [MainLayout.tsx](../../frontend/src/components/layout/MainLayout.tsx) |
| App 路由 | [App.tsx](../../frontend/src/App.tsx) |
| Swagger UI HTML | [app.py](../../backend/xianyu_hunter/web/app.py) |
| 鉴权白名单 | [auth.py](../../backend/xianyu_hunter/web/middleware/auth.py) |
| 版本号源 | [\_\_init\_\_.py](../../backend/xianyu_hunter/__init__.py) |

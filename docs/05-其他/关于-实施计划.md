...
> ⚠️ **已废弃**：本文档为历史实施计划文档，所列 15 个 Task 均已实施完成，但实施过程中部分细节（如 check-update 接入 GitHub Releases API、BrandCard 新增 publishedAt Tooltip、依赖列表实际为 42 项而非示例的 8 项等）与最新代码实现不一致。当前最新实现以 [`关于-详细设计.md`](./关于-详细设计.md) 为准，请优先参考该文档。
...
**关联设计文档:** [关于-概要设计.md](../关于-概要设计.md)
**关联需求文档:** [关于-需求规格.md](../关于-需求规格.md)
...
| 文件 | 职责 | 操作 |
|------|------|------|
| `backend/xianyu_hunter/_build_info.py` | 构建时生成的版本元信息 | 新建（构建脚本写入） |
| `backend/xianyu_hunter/web/routes/api_about.py` | About API 路由 | 新建 |
| `backend/xianyu_hunter/web/app.py` | include_router + Swagger UI 顶栏改跳转 | 修改 |
| `backend/xianyu_hunter/web/middleware/auth.py` | 鉴权白名单新增 2 项 | 修改 |
...
**Files:**
- Create: `backend/xianyu_hunter/web/routes/api_about.py`
- Modify: `backend/xianyu_hunter/web/app.py`（`include_router` 列表）
- Modify: `backend/xianyu_hunter/web/middleware/auth.py`（`WHITELIST` 集合）

- [ ] **Step 1.1: 创建 `api_about.py`**

新建 `backend/xianyu_hunter/web/routes/api_about.py`：
...
修改 `backend/xianyu_hunter/web/app.py` 的路由 include 列表（参考第 159-186 行的 `app.include_router(...)` 调用），在 `api_auth` 之后追加：
...
修改 `backend/xianyu_hunter/web/middleware/auth.py` 第 32 行附近的 `WHITELIST` 集合，追加：
...
```bash
git add backend/xianyu_hunter/web/routes/api_about.py backend/xianyu_hunter/web/app.py backend/xianyu_hunter/web/middleware/auth.py
...
Run: `Get-Content d:\code\otherProjects\17_xianyu\backend\xianyu_hunter\_build_info.py`
...
```bash
git add scripts/build_info.py scripts/重新构建.bat backend/xianyu_hunter/_build_info.py
...
**Files:**
- Modify: `backend/xianyu_hunter/web/app.py:355`

- [ ] **Step 3.1: 改跳转链接**

修改 `backend/xianyu_hunter/web/app.py` 第 355 行附近：
...
Run: `Get-Content d:\code\otherProjects\17_xianyu\backend\xianyu_hunter\web\app.py | Select-String "app/about"`
...
```bash
git add backend/xianyu_hunter/web/app.py
...
Run: `cd d:\code\otherProjects\17_xianyu; $env:PYTHONPATH="src"; python -m xianyu_hunter web --port 8001`

Expected: 服务在 8001 端口启动，Swagger UI 可访问 `http://127.0.0.1:8001/api/docs`

- [ ] **Step 15.2: 验证 API**

Run: `curl http://127.0.0.1:8001/api/about`（PowerShell: `Invoke-RestMethod http://127.0.0.1:8001/api/about`）

Expected: 返回 JSON 含 version / build_date / git_sha

Run: `Invoke-RestMethod http://127.0.0.1:8001/api/about/check-update`

Expected: `has_update: false`

- [ ] **Step 15.3: 验证 Swagger 顶栏跳转**

浏览器访问 `http://127.0.0.1:8001/api/docs` → 点击右上角"📖 关于"按钮
...
## 实施内容
- 新增 13 个文件、修改 5 个文件（详见 关于-概要设计.md §3.2）
- P0-P2 全量完成

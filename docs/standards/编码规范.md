...
> **适用范围**：`backend/xianyu_hunter/`（Python）+ `frontend/src/`（TypeScript）
> **配套技能**：
...
| 层级 | 文档 | 何时查阅 |
|---|---|---|
| **总入口（本文档）** | `standards/编码规范.md` | 任何编码前必读 |
...
| 路径 | 用途 |
|---|---|
| `backend/xianyu_hunter/domain/` | 纯数据模型 |
| `backend/xianyu_hunter/infra/` | 基础设施（DB、浏览器、日志、密钥、仓储） |
| `backend/xianyu_hunter/modules/` | 业务模块（采集器、通知器、买家、评估器、聊天机器人） |
| `backend/xianyu_hunter/web/routes/` | FastAPI 路由（`api_*.py`） |
| `backend/xianyu_hunter/web/middleware/` | Web 中间件（认证、错误捕获、请求 ID） |
| `frontend/src/api/` | API 客户端 |
| `frontend/src/stores/` | Zustand 状态 |
| `frontend/src/pages/` | 页面组件（按业务模块分子目录） |
| `frontend/src/mobile/` | 移动端专属组件与页面 |

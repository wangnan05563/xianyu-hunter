...
### Q1：这个规划什么时候实施？
**A**：本规划为设计阶段产出，5 大类 15 个子功能按优先级分批实施。P0 项（首屏引导、自动聚焦、快捷回复）优先；P1 项（跨会话搜索、消息撤回、1-5 星评分）次之；P2 项（多设备同步、云端存储）为后续扩展。

### Q2：为什么不直接引入第三方聊天 UI 库？
**A**：遵循项目设计原则——简约至上。现有 SessionList、AssistantMessage、EmptyIllustration 等组件已满足核心需求，引入第三方库会增加依赖体积、样式冲突风险和定制成本。antd 已覆盖大部分 UI 需求。

### Q3：首屏引导功能如何实现？
**A**：基于现有 EmptyIllustration 组件扩展，新增功能介绍卡片（3-4 张可滑动卡片）、常见问题快捷入口（3-5 个高频问题按钮）、使用指南提示（首次访问时显示）。所有 UI 沿用 `--cb-pink-light/-deep` `--cb-cyan` 马卡龙色板。

### Q4：跨会话搜索需要后端支持吗？
**A**：需要。当前后端仅支持单会话内消息列表查询（`before_id` 游标分页），跨会话搜索需新增 API 端点，支持按关键词搜索所有会话的消息内容。建议使用 SQLite FTS5 全文索引优化搜索性能。

### Q5：消息撤回功能的实现方案？
**A**：前端在消息发送后短暂显示"撤回"按钮（3-5 秒），后端提供 `DELETE /api/chatbot/messages/{message_id}` 端点。撤回后消息从 `chatbot_messages` 表软删除（标记 deleted），前端隐藏该消息并显示"消息已撤回"提示。

### Q6：为什么要做图文混排气泡？
**A**：当前图片上传后独立显示，与文字消息分离。图文混排可以让用户在同一气泡中发送"图片+文字说明"，提升信息密度和可读性，类似主流 IM 工具的体验。技术实现上，前端需支持 `<img>` 标签在 Markdown 渲染中的行内展示。

### Q7：1-5 星评分与现有正/负反馈如何共存？
**A**：两者互补：正/负反馈（点赞/点踩）用于快速评价，1-5 星评分用于精细化评价。前端在点踩后弹出星级选择器，用户可选填 1-5 星和文字反馈。后端 `chatbot_feedback` 表新增 `rating_star` 字段（1-5）。

### Q8：本地存储是什么？为什么需要？
**A**：将部分会话数据缓存到浏览器 localStorage，减少网络请求，提升离线体验。包括：最近会话列表、草稿消息、用户偏好设置（如工具开关默认值）。注意：本地存储仅作缓存，数据以服务端为准。

### Q9：快捷回复功能如何设计？
**A**：在输入框上方或侧边提供 3-5 个常用问题按钮（如"如何创建任务？""价格策略怎么设置？""通知渠道有哪些？"），点击后自动填入输入框并发送。问题列表从 FAQ 库中动态获取高频问题。

### Q10：这个规划与项目硬约束有冲突吗？
**A**：没有。所有设计严格遵守项目硬约束：不引入新 UI 库（复用 antd）、不引入角色权限（单用户场景）、后端优先复用现有路由、云端同步仅作后续扩展。所有新增 UI 沿用马卡龙风格 CSS Token。

...
class RepositoryBase:
    ...
    def db_count_by_predicate(
        ...
        *,
        ...
        性能优化：原实现发 2 次 COUNT 查询（matched + total），business_kpi 一次
        请求 6 次调用 = 12 次 COUNT。现用 CASE WHEN 聚合合并为 1 次查询。
        """
        col: Any = getattr(row_cls, time_col)
        with self.engine.connect() as conn:
            if extra_where:
                # 用 CASE WHEN 一次查询同时取 matched 和 total
                # matched = 满足 extra_where 的行数，total = 时间窗内所有行数
                ...
                # 无 extra_where 时 matched == total，一次 COUNT 即可
                total = int(
                    conn.execute(
                        select(func.count())
                        .select_from(row_cls)

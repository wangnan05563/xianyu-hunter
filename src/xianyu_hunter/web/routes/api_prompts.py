...
# 智能客服模块 Prompt 扩展：条件性添加，chatbot 依赖缺失时不暴露
# 为什么用 try/except 而非顶层 import：chromadb/openai 未安装时
# RAGEngine/Agent 类仍可导入（vector_store 延迟导入 chromadb），
# 但保险起见仍捕获异常，确保 prompts 页面在 chatbot 不可用时正常工作
try:
    from xianyu_hunter.modules.chatbot.rag_engine import RAGEngine as _RAGEngine
    from xianyu_hunter.modules.chatbot.agent import Agent as _Agent
    _PROMPTS_META["chatbot_rag"] = {
        "filename": "chatbot_rag.txt",
        "description": "智能客服 RAG 引擎系统提示词（基于文档片段回答）",
        "default": _RAGEngine._SYSTEM_PROMPT,
    }
    _PROMPTS_META["chatbot_agent"] = {
        "filename": "chatbot_agent.txt",
        "description": "智能客服 AGENT 系统提示词（工具调用模式）",
        "default": _Agent._SYSTEM_PROMPT,
    }
except ImportError:
    pass
...
def _reload_prompt_to_memory(key: str, content: str) -> None:
    """将新 prompt 内容同步到对应模块的内存变量

    为什么需要这步：各模块直接引用模块级/类级 _SYSTEM_PROMPT 变量，
    修改文件后需同步内存变量才能立即生效（无需重启进程）。
    - api_ai: 模块级变量直接赋值
    - chatbot: 类常量通过类名赋值（所有实例共享，下次调用即生效）
    """
    ...
    elif key == "chatbot_rag":
        # 修改类常量：RAGEngine._build_messages 每次调用都读取 _SYSTEM_PROMPT，
        # 修改类常量后下一次 generate() 即使用新 prompt
        try:
            from xianyu_hunter.modules.chatbot.rag_engine import RAGEngine
            RAGEngine._SYSTEM_PROMPT = content
        except ImportError:
            pass  # chatbot 模块不可用时静默忽略
    elif key == "chatbot_agent":
        try:
            from xianyu_hunter.modules.chatbot.agent import Agent
            Agent._SYSTEM_PROMPT = content
        except ImportError:
            pass

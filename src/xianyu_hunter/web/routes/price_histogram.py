...
def _build_compare_means(
    ...
    if not all_rows_with_ts:
        return {"yesterday": 0.0, "last7d": 0.0, "diff_pct": 0.0}
    ...
    return {"yesterday": y_mean, "last7d": w_mean, "diff_pct": diff}
...
def prices_histogram(
    ...
    with engine.connect() as conn:
            ...
            if row:
                ...
                return {
                    ...
                    "summary": {
                        ...
                        "p25": 0, "p75": 0,
                        "compare": {"yesterday": 0, "last7d": 0, "diff_pct": 0},
                    },
        ...
        if scope["mode"] == "task":
            rows = conn.execute(select(ItemRow.price).where(ItemRow.task_id == task_id)).all()
        else:
            rows = conn.execute(select(ItemRow.price)).all()
